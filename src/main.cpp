...
int main(void){
	...
	while((cvWaitKey(5) & 0xff) != 27)
	{
		...
		if (!res) {
            ...
            slMat2cvMat(zed->retrieveImage(static_cast<sl::zed::SIDE> (1))).copyTo(image2);
            
            int height = l.height;
            
           	resize(image1, image1, Size(), 480.0f/height, 480.0f/height, INTER_AREA);
			resize(image2, image2, Size(), 480.0f/height, 480.0f/height, INTER_AREA);
            //std::cout << image1.size().width << endl;
			cvtColor(image1(Rect(106,0,640,480)), image1, CV_BGR2GRAY);
			cvtColor(image2(Rect(106,0,640,480)), image2, CV_BGR2GRAY);
	...
	matToPointXYZ(right, disp32, cloud);
